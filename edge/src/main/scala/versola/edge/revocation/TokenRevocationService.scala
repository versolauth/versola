...
object TokenRevocationService:
  def live: ZLayer[RevocationRepository & RevocationNotifications & EdgeConfig & Scope, Nothing, TokenRevocationService] =
    ZLayer.fromZIO:
      for
  ...
  private[revocation] def make(repository: RevocationRepository): UIO[Impl] =
    for
      entries <- TMap.empty[RevocationKey, Revocation].commit
      now <- Clock.instant
      lastReload <- Ref.make(now)
    yield Impl(repository, entries, lastReload)
