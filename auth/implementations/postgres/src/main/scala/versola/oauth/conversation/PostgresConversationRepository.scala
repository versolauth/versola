...
import versola.oauth.client.model.{Acr, AuthFlow, ClientId, PassedAuthFactor, PassedFactorRecord, RegistrationFlow, ResourceUri, ScopeToken}
...
class PostgresConversationRepository(xa: TransactorZIO) extends ConversationRepository, BasicCodecs:
  ...
  given DbCodec[UserAgentCookiePayload] = jsonBCodec[UserAgentCookiePayload]
  ...
  override def create(authId: AuthId, record: ConversationRecord, ttl: Duration): Task[Unit] =
    xa.connectMeasured("create-conversation") {
      sql"""insert into auth_conversations (
                ...
                ${record.userClaims},
                ${record.authFlow},
                ${record.registrationFlow},
                ${record.registrationStep},
                ${record.userAgent},
                ${record.userAgentCookie},
                ${record.version},
                ${record.amr},
  ...
  override def overwrite(authId: AuthId, record: ConversationRecord): Task[Boolean] =
    xa.connectMeasured("overwrite-conversation") {
      sql"""update auth_conversations set
              ...
              registration_flow = ${record.registrationFlow},
              registration_step = ${record.registrationStep},
              amr = ${record.amr},
