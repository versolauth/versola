CREATE TABLE refresh_tokens(
    ...
    -- Names the rotation chain this token belongs to. Random, generated by the caller when a
    -- chain starts, and unrelated to any token value in it -- nothing authenticates by
    -- presenting it, so it is an identity rather than a credential. Kept on rotated-away rows
    -- so a token replayed any number of generations later still resolves to the family that
    -- has to be revoked.
