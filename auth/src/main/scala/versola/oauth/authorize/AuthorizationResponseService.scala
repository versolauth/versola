...
object AuthorizationResponseService:
  ...
  /** JARM §4.1 requires an `exp`, and the response is redeemed by the browser redirect that
    * carries it -- so it only has to outlive that hop, not the code it delivers.
    */
  ...
  class Impl(
    ...
    /** JARM §4.1: the response parameters become claims of a JWT issued by this server to the
