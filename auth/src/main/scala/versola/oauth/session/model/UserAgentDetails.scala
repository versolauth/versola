...
object UserAgentDetails:
  private val unknown = UserAgentDetails(None, None, None, None)
  ...
  def parse(userAgent: Option[String]): UserAgentDetails =
    userAgent.map(_.trim).filter(_.nonEmpty).fold(unknown)(parse)

  def parse(ua: String): UserAgentDetails =
    val platform =
      if iosRe.findFirstMatchIn(ua).isDefined then Some("ios")
      else if androidRe.findFirstMatchIn(ua).isDefined then Some("android")
      else if desktopRe.findFirstMatchIn(ua).isDefined then Some("desktop")
      else None
    val os =
    ...
    val (browser, version) =
      edgeRe.findFirstMatchIn(ua).map(m => ("Edge", Option(m.group(1))))
        ...
        .map((b, v) => (Option(b), v)).getOrElse((None, None))
    UserAgentDetails(platform, os, browser, version)