...
object OAuthClientServiceSpec extends UnitSpecBase:
  ...
  // Generate proper 32-byte secrets
  ...
  val testClientSecret = Secret(testSecretBytes)
  val previousClientSecret = Secret(previousSecretBytes)
  val wrongClientSecret = Secret(wrongSecretBytes)
  ...
  class Env:
    val cache = stub[Ref[Map[ClientId, OAuthClientRecord]]]
    val repository = stub[OAuthClientRepository]
    val scopeCache = stub[Ref[Map[ScopeToken, OAuthScope]]]
    ...
    suite("find")(
      test("return Some when client exists") {
        ...
        for
          ...
          result <- env.service.findCached(clientId1)
      ...
      test("return None when client does not exist") {
        ...
        for
          _ <- (env.cache.get(using _: Trace)).succeedsWith(testClients)
          result <- env.service.findCached(nonExistentClientId)
        yield assertTrue(result.isEmpty)
      },
  ...
  def registerTests =
    suite("register")(
      test("create private client with generated secret") {
        ...
        val testMac = MAC.fromBase64Url("jzaMcn2zEvgXA7UoCh86TKMrhSKCZ84LTEG3Xx4F0jc").toOption.get
        val expectedMacWithSalt = Secret(testMac ++ saltBytes)
        val expectedSecret = Secret(secretBytes)
        ...
        yield assertTrue(
          result.sameElements(expectedSecret),
          ...
          registerCalls.head.secret.exists(_.sameElements(expectedMacWithSalt)),
    ...
    suite("rotateSecret")(
      test("generate new secret and update repository") {
        ...
        yield assertTrue(
          ...
          rotateCalls.head._2.sameElements(expectedMacWithSalt),
        )
      },
    )
