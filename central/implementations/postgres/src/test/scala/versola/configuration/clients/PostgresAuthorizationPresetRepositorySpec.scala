  ...
  override def beforeEach(env: AuthorizationPresetRepositorySpec.Env) =
    ZIO.serviceWithZIO[TransactorZIO] { xa =>
      xa.connect(sql"TRUNCATE TABLE tenants RESTART IDENTITY CASCADE".update.run()) *>
        ...
        xa.connect(sql"INSERT INTO oauth_clients (tenant_id, id, client_name, redirect_uris, scope, access_token_ttl, refresh_token_ttl, permissions, theme, auth_flow, otp_template_id) VALUES ('tenant-a', 'client-1', '{\"en\":\"Client 1\"}', '{}', '{}', 300, 7776000, '{}', 'default', '{\"primary\":{\"credentials\":[\"phone\"],\"inlinePassword\":false,\"factors\":[{\"type\":\"otp\",\"required\":true}]}}'::jsonb, 'default')".update.run()) *>
