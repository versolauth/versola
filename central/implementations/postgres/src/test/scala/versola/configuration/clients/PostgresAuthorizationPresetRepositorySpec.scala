  ...
  override def beforeEach(env: AuthorizationPresetRepositorySpec.Env) =
    ZIO.serviceWithZIO[TransactorZIO] { xa =>
      xa.connect(sql"TRUNCATE TABLE tenants RESTART IDENTITY CASCADE".update.run()) *>
        ...
        xa.connect(sql"INSERT INTO otp_templates (id, tenant_id, localizations) VALUES ('default-otp', 'tenant-a', '{}')".update.run()) *>
        xa.connect(sql"INSERT INTO oauth_clients (tenant_id, id, client_name, redirect_uris, scope, external_audience, access_token_ttl, refresh_token_ttl, permissions, theme, auth_flow, otp_template_id) VALUES ('tenant-a', 'client-1', 'Client 1', '{}', '{}', '{}', 300, 7776000, '{}', 'default', '{\"primary\":{\"credentials\":[\"phone\"],\"inlinePassword\":false,\"factors\":[{\"type\":\"otp\",\"required\":true}]}}'::jsonb, 'default-otp')".update.run()) *>
        xa.connect(sql"INSERT INTO oauth_clients (tenant_id, id, client_name, redirect_uris, scope, external_audience, access_token_ttl, refresh_token_ttl, permissions, theme, auth_flow, otp_template_id) VALUES ('tenant-a', 'client-2', 'Client 2', '{}', '{}', '{}', 300, 7776000, '{}', 'default', '{\"primary\":{\"credentials\":[\"phone\"],\"inlinePassword\":false,\"factors\":[{\"type\":\"otp\",\"required\":true}]}}'::jsonb, 'default-otp')".update.run())
    }.unit
